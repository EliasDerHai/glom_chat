...
var ApiReturnedTodos = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var ApiUpdatedTodo = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserClickedComplete = class extends CustomType {
      ...
      return field(
        ...
        (title) => {
          return field(
            ...
            bool,
            (completed) => {
              return success(new Todo(id, title, completed));
            }
          );
        }
      );
...
function fetch_todos(handle_response) {
  let url = "https://jsonplaceholder.typicode.com/todos/";
  let _block;
  let _pipe = list2(todo_decoder());
  _block = map2(
    _pipe,
    (_capture) => {
...
function init(_) {
  let model = toList([]);
  let effect = fetch_todos((var0) => {
    return new ApiReturnedTodos(var0);
  });
  return [model, effect];
}
function complete_todo(id, completed, handle_response) {
  let url = "https://jsonplaceholder.typicode.com/todos/" + to_string(id);
  let handler = expect_json(success(id), handle_response);
  let body = object2(toList([["completed", bool2(completed)]]));
  let $ = to(url);
  if ($ instanceof Ok) {
    let request = $[0];
    let _pipe = request;
    let _pipe$1 = set_method(_pipe, new Patch());
    let _pipe$2 = set_body(_pipe$1, to_string3(body));
    return send3(_pipe$2, handler);
  } else {
    throw makeError(
      "panic",
      FILEPATH,
...
function update2(model, msg) {
  if (msg instanceof ApiReturnedTodos) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let todos = $[0];
      return [todos, none()];
  ...
  } else if (msg instanceof ApiUpdatedTodo) {
    let $ = msg[0];
    if ($ instanceof Ok) {
      let id = $[0];
      let todos = map(
        ...
        (item) => {
          let $1 = item.id === id;
          if ($1) {
            return new Todo(item.id, item.title, !item.completed);
          } else {
            return item;
          }
        }
    ...
    return [
      ...
      complete_todo(
        ...
        completed,
        (var0) => {
          return new ApiUpdatedTodo(var0);
        }
      )
    ];
  }
}
function view_todo(item, handle_complete) {
  return label(
    toList([class$("flex gap-2 items-baseline")]),
    toList([
      p(
        toList([
          class$("flex-1"),
          classes(
            toList([["line-through text-slate-400", item.completed]])
          )
        ]),
...
function view(model) {
  return div(
    toList([class$("p-32 mx-auto w-full max-w-2xl space-y-8")]),
    toList([
      h1(
            ...
            let html = li(
              ...
              toList([
                view_todo(
                  item,
                  (_capture) => {
                    return new UserClickedComplete(item.id, _capture);
                  }
                )
              ])
            );
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 571, end: 620, pattern_start: 582, pattern_end: 587 }
    );
  }
  return void 0;
}
